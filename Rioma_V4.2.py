...
class Toad:
    def __init__(self,vie=6,nom="Mario",taille=90,speed=4,jump=15,song_jump='',direction=-1,imgD='',imgG='',imgDH='',imgGH='',gravity=0.5):
        self.vie = vie
        self.nom = nom
        self.taille = taille
        self.speed = speed
...
toad_x, toad_y = width - Toad.taille - 50, height - Toad.taille - 100
...
def Jouer():
    ...
    while running:
        ...
        if toad_x <= 0 or toad_x >= width - Toad.taille:
            Toad.direction *= -1
        
        if (player_x + Joueur.taille > toad_x and player_x < toad_x + Toad.taille and player_y + Joueur.taille > toad_y and player_y < toad_y + Toad.taille) and tpsTM+3000 < tpsTS:
            Toad_spawn = False
            toad_x, toad_y = width - Toad.taille - 50, height - Toad.taille - 100
            Nb_Toad += 1
